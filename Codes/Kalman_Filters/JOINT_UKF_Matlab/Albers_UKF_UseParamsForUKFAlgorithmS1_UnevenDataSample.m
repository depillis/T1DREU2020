...
NoIdealSamples=350;
MaxTimeStep=7; %LdeP maybe up to 7 minutes between samples
Trandstepvec=ceil(randspace(0,NoIdealSamples,timeFinal,[1,MaxTimeStep]));
NumSamples=numel(Trandstepvec); %LdeP Because we don't know how many samples we'll actually get
% Trandstepvec = ceil(randspace(0,NoSamples,[],[1,MaxTimeStep])); %LdeP don't put an upper bound on the final time
...
ukf.ProcessNoise = sqrt(Rv)/1000; %LdeP Making this smaller and also proportional to stdev instead of cov to adhere to the ODE model more than to the random data
...
% LdeP Trandstepvec is the vector of NumSamples randomly spaced sample times
...
%DelT(NumSamples)=1; %LdeP Fake final time step to make DelT correct dimension
