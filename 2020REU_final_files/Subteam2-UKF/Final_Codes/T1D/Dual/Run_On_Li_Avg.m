%DOES PARAMETER ESTIMATION USING DUAL UKF ON AVERAGED VERSION OF LI ET AL
%DATA
%THE AVERAGED DATASET WAS CREATED BY SUBTEAM 1.
...
Npar = 41;
figure_params = zeros(Npar + 1, 1); %this will hold the best fit parameter values for each mouse
...
sim_params = figure_params(2:42);
options=odeset('RelTol',1e-12,'AbsTol',1e-12);
sim_sol = ode15s(@(t, y) fig_2_3_4_5_ODE(t, y, f1n, f2n, wave, sim_params), [0 350], InitialState,options); %Simulate ODE's up until first time point
x_sim = zeros(12, 350);
for j = 1:350
    x_sim(:,j) = deval(sim_sol, j);
end
figure(1);
p1 = plot(1:350, x_sim(6,:), 'r', 'Linewidth', 2); hold on;
p2 = plot(7 * X(1,:), X(2,:), 'ko', 'MarkerFaceColor', 'k', 'Linewidth', 2); hold off;
    
csv_save = [sim_sol.x' sim_sol.y(6,:)'];
