...
#proteins<-data.matrix(read.csv("noise data 0.5%.csv", sep="\t"))
proteins<-data.matrix(read.csv("noise data 0.5%.csv", row.names=1))
...
sigma.data<-data.matrix(read.csv("variance noise data 0.5%.csv", row.names=1))

prior.solution.steady<-data.matrix(read.csv("/R codes model/Model 2/0.5/set parameter solutions_egf path steady state daniel.csv", row.names=1))
prior.solution.all<-data.matrix(read.csv("/R codes model/Model 2/0.5/set parameter solutions_egf path all points.csv", row.names=1))
...
save(all.data,file='input.data.b.RData')
