...
prior.solution.steady<-data.matrix(read.csv("/R codes model/Model 2/5.0/set parameter solutions_egf path steady state daniel.csv", row.names=1))
prior.solution.all<-data.matrix(read.csv("/R codes model/Model 2/5.0/set parameter solutions_egf path all points.csv", row.names=1))
...
save(all.data,file='input.data.d.RData')
