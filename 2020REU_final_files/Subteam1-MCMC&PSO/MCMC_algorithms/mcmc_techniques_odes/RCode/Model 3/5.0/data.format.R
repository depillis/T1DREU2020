#setwd("/R codes model/Model 3/5.0")
#source('data.format.R')


proteins<-data.matrix(read.csv("noise data 5%.csv", row.names=1))
...
sigma.data<-data.matrix(read.csv("variance noise data 5%.csv", row.names=1))

prior.solution.steady<-data.matrix(read.csv("/R codes model/Model 3/5.0/set parameter solutions_egf path steady state daniel.csv", row.names=1))
prior.solution.all<-data.matrix(read.csv("/R codes model/Model 3/5.0/set parameter solutions_egf path all points.csv", row.names=1))
...
save(all.data,file='/R codes model/Model 3/data/input.data.d.RData')
