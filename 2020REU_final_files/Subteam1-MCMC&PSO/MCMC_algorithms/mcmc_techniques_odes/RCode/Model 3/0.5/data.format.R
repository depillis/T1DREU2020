...
prior.solution.steady<-data.matrix(read.csv("/R codes model/Model 3/0.5/set parameter solutions_egf path steady state daniel.csv", row.names=1))
prior.solution.all<-data.matrix(read.csv("/R codes model/Model 3/0.5/set parameter solutions_egf path all points.csv", row.names=1))
...
save(all.data,file='/R codes model/Model 3/data/input.data.b.RData')
