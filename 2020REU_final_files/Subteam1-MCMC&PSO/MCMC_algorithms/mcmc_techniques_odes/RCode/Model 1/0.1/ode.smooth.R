...
original.data<-read.csv("noise data 0.1%.csv",row.names=1)
...
#pdf("smoothing data1.pdf")
#pdf("smoothing data2.pdf")
#pdf("smoothing data3.pdf")
#pdf("smoothing data egf2.pdf")
pdf("smoothing data.pdf")
par(mfrow=c(2,2))
for(i in 2:cols)
{
	...
	#using log
	#approxi.list<-approx(log(original.data2[,1]),original.data2[,i],n=(100/0.01+1))  #n=100/0.01+1
#	approxi.list<-approx(smooth.data2[,1],smooth.data2[,data.names[i]],n=(100/0.01+1))  #n=100/0.01+1
	approxi.list<-approx(smooth.data[,1],smooth.data2[,data.names[i]],n=((100-0)/0.01+1))  #n=100/0.01+1
	#approxi.list<-approx(smooth.data2[,1],smooth.data2[,data.names[i]],n=(100/0.01+1))  #n=100/0.01+1
	#without log
	#approxi.list<-approx(smooth.data[,1],smooth.data[,data.names[i]],n=(100/0.01+1))  #n=100/0.01+1
