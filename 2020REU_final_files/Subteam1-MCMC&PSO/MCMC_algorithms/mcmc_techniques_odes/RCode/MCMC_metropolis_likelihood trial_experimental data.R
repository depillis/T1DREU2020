...
n.part<-paste('metropolis',likelihood.choice,sep='')

n.l<-'realistic'
k<-4    #k=1:5, for the five models								## variable for selected model
...
ptm<-proc.time()
all.samp<-metrop(calling.function_a, initial = logik,nbatch=1001000, blen = 1, nspac = 1, scale = 0.01, debug = TRUE)
ptm2<-proc.time()-ptm
