...
function [funcSamp] = metroMCMC(thetaInit, Nburnin, Nsamp, verbose, sampleCov, obsdata, Ndat)
    ...
    funcCur=logpostlinearmodel(thetaCur,obsdata, Ndat); % in this case calls logpostlinearmodel: 
    ...
    for n=1:nb
        ...
        %   distributions
        a0Prop = mvnrnd(thetaCur,sampleCov); % giving us a new proposed paramters (theta)
        aProp = mvnrnd(thetaCur,sampleCov);
        sigProp = mvnrnd(thetaCur,sampleCov);
        thetaProp = [a0Prop(1) aProp(2) sigProp(3)];
        %   using prior (covariance matrix)
        funcProp=logpostlinearmodel(thetaProp, obsdata, Ndat); % gives a new posterior distribution using the new proposed theta
