...
function [T, Y] = T1Dfun_fitAllLi(params, waveon, type, wICs, time)
...
switch type
    ...
    case 'NOD'
        ...
        if waveon == 1
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys_fitAllLi(t, y, f1n, f2n, wave, params), [0 time(1)], IC, options);
            IC = IC(end, :);  
            [T, Y] = ode15s(@(t, y) T1Dsys_nfitAllLi(t, y, f1n, f2n, wave, params), time, IC, options);
        % Solve w/o apoptotic wave
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys_fitAllLi(t, y, f1n, f2n, 0, params), [0 time(1)], IC);
            IC = IC(end, :);
            [T, Y] = ode15s(@(t, y) T1Dsys_fitAllLi(t, y, f1n, f2n, 0, params), time, IC);
        end
    ...
    otherwise
        ...
        if waveon == 1
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys_fitAllLi(t, y, f1, f2, wave, params), [0 time(1)], IC);
            IC = IC(end, :);
            [T, Y] = ode15s(@(t, y) T1Dsys_fitAllLi(t, y, f1, f2, wave, params), time, IC);
        % Solve w/o apoptotic wave
        else
            if wICs == 1
                IC = params(47:end-2);  
            ...
            [~, IC] = ode15s(@(t, y)T1Dsys_fitAllLi(t, y, f1, f2, 0, params), [0 time(1)], IC);
            IC = IC(end, :);
            [T, Y] = ode15s(@(t, y) T1Dsys_fitAllLi(t, y, f1, f2, 0, params), time, IC);
        end
end
end