...
function [Tpre, Ypre, Tpost, Ypost] = T1Dfun(params, waveon, type, wICs, datalen)
...
switch type
    ...
    case 'NOD'
        ...
        if waveon == 1
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys(t, y, f1n, f2n, wave, params), 0:Tspan_pre(1), IC, options);
            IC = IC(end, :);
            options = odeset('Events', @sickEvents);  
            [Tpre, Ypre] = ode15s(@(t, y) T1Dsys(t, y, f1n, f2n, wave, params), Tspan_pre, IC, options);
            [Tpost, Ypost] = ode15s(@(t, y) T1Dsys(t, y, f1n, f2n, wave, params), Tspan_post, Ypre(end, :), options);
        % Solve w/o apoptotic wave
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys(t, y, f1n, f2n, 0, params), 0:Tspan_pre(1), IC);
            IC = IC(end, :);
            [Tpre, Ypre] = ode15s(@(t, y) T1Dsys(t, y, f1n, f2n, 0, params), Tspan_pre, IC);
            [Tpost, Ypost] = ode15s(@(t, y) T1Dsys(t, y, f1n, f2n, 0, params), Tspan_post, Ypre(end, :));
        end
    ...
    otherwise
        ...
        if waveon == 1
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys(t, y, f1, f2, wave, params), Tspan_pre, IC);
            IC = IC(end, :);
            [Tpre, Ypre] = ode15s(@(t, y) T1Dsys(t, y, f1, f2, wave, params), Tspan_pre, IC);
            [Tpost, Ypost] = ode15s(@(t, y) T1Dsys(t, y, f1, f2, wave, params), Tspan_post, Ypre(end, :));
        % Solve w/o apoptotic wave
        else
            if wICs == 1
                IC = params(47:end-2);  
            ...
            [~, IC] = ode15s(@(t, y) T1Dsys(t, y, f1, f2, 0, params), 0:Tspan_pre(1), IC);
            IC = IC(end, :);
            [Tpre, Ypre] = ode15s(@(t, y) T1Dsys(t, y, f1, f2, 0, params), Tspan_pre, IC);
            [Tpost, Ypost] = ode15s(@(t, y) T1Dsys(t, y, f1, f2, 0, params), Tspan_post, Ypre(end, :));
        end
end
end
