...
This folder contains the .csv files for aligning and averaging the acute Li et al data 
sets. It also contains several figures of the averaged data.

Files marked with * appear in the final REU write up

------------------------------------------------------------------------------------------
FILES

* acute_pivotPoints.fig/acute_pivotPoints.png: plot of each of the 9 acute mice with "pivot 	
	points" (chosen point of alignment) designated

avg_lietal_2.csv: raw averaged acute data with time in weeks

* avgData.fig/avgData.png: plot of the averaged data (from avg_lieta_2.csv) time converted 	
	into days

consolidated_Lietal.xlsx: contains sheets that show the process of shifting and averaging
 	the acute mouse data
	- absTime_alignment: aligning 11 mice into absolute time
	- pivot_alignment: determining pivot point, aligning according to pivot points, a
		averaging glucose and time data

lietal_meandat.m: our first attempt to create a function that aligns and averages the Li
 	et al data at the onset of diabetes point. However, for our tutorial we have done 
 	this alignment by hand.
