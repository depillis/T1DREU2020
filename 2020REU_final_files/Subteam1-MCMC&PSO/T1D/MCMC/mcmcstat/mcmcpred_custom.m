function out=mcmcpred_custom(results,chain,s2chain,data,modelfun,nsample,modeltype,parVer,wave,varargin)
...
for i=1:nbatch
  ...
  for iisample = 1:nsample
    ...
    y   = feval(modelfun,modeltype,parVer,wave,th,datai,varargin{:});
    ysave(iisample,:,:) = y;
    if ~isempty(s2chain)
      if sstype==0
        osave(iisample,:,:) = ...
