function [results,chain,s2chain,sschain, hchain]=mcmcrun_custom(model,data,params,options,modtype,paramver,wOn,res)
...
oldpar=par0(:)';
ss = sseval(ssfun,ssstyle,oldpar,parind,value,local,data,modelfun,modtype,paramver,wOn);
ss1 = ss;
ss2 = ss;
...
for isimu=2:nsimu % simulation loop
  ...
  if any(newpar<low(parind)) || any(newpar>upp(parind))
    ...
    % calculate ss
    ss2 = ss;             % old ss
    ss1 = sseval(ssfun,ssstyle,newpar,parind,value,local,data,modelfun,modtype,paramver,wOn);
  ...
  if dodram == 1 && accept == 0 % & outbound == 0
    ...
    while accept == 0 && itry < Ntry
      ...
      outbound = 0;
      z.ss = sseval(ssfun,ssstyle,z.p,parind,value,local,data,modelfun,modtype,paramver,wOn);
...
if dostats && (saveit == 1 || savesize >= nsimu)
  ...
  if not(savepostinss) % can not do if this is set
    ss = sseval(ssfun,ssstyle,meanchain,parind,value,local,data,modelfun,modtype,paramver,wOn);
    D = mean(sschain);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ss = sseval(ssfun,ssstyle,theta,parind,value,local,data,modelfun,modtype,paramver,wOn)
% evaluate the "sum-of-squares" function
value(parind) = theta;
if ssstyle == 1
  ss = feval(ssfun,value(:)',data,modtype,paramver,wOn);
elseif ssstyle == 4
